...
for node in workflow.get('nodes', []):
    ...
    if node_id in {'format_rag_reply', 'format_general_reply'}:
        ...
        if 'const debugEnabled = true;' in js:
            params['jsCode'] = js.replace('const debugEnabled = true;', 'const debugEnabled = false;')
            changed += 1
...
python3 -m json.tool "$WORKFLOW_FILE" >/tmp/rag-query-after-disable-debug.json
...
echo "[OK] Routing debug markers disabled and workflow redeployed."
