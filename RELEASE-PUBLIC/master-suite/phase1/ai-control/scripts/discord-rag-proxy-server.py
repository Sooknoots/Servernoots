...
class Handler(BaseHTTPRequestHandler):
    server_version = "DiscordRagProxy/1.0"
    ...
    def do_POST(self) -> None:
        if self.path not in {"/proxy", "/discord-rag"}:
        ...
        data = self.rfile.read(max(0, raw_len))
        try:
        ...
        try:
            ...
            allowlisted, allow_reason = allowlist_decision(
                event,
                allowed_guild_ids=self.server.allowed_guild_ids,
                allowed_channel_ids=self.server.allowed_channel_ids,
                allowed_role_ids=self.server.allowed_role_ids,
            )
            if not allowlisted:
                denial = {
            ...
            if command == "status":
                status_payload = build_status_payload(
                    event,
                    n8n_base=self.server.n8n_base,
                    health_path=self.server.n8n_health_path,
            ...
            if is_voice_command(command):
                voice_state = load_voice_state(self.server.voice_state_file)
                allowed, retry_after = voice_cooldown_check(
                    state=voice_state,
                    event=event,
                    command=command,
                    cooldown_seconds=self.server.voice_cooldown_seconds,
                    moderator_role_ids=self.server.voice_moderator_role_ids,
                )
                if not allowed:
                    denial = {
                    ...
                    self._json(429, denial)
                    return

                profile_ctx = build_profile_context(
                    profiles=self.server.profiles,
                    user_id=user_id,
                    interaction_user_id=interaction_user_id,
                    active_user_ids=active_user_ids,
                    allow_force=bool(self.server.allow_force),
                )
                memory_state = load_memory_state(self.server.memory_state_file)
                memory_ctx = memory_context_for_event(event, user_id, memory_state, self.server.memory_policy)
                payload = build_payload(event, profile_ctx, command=command, message=normalized_message, memory_ctx=memory_ctx)
                memory_persisted = False
                if self.server.voice_forward:
                    result = post_json(self.server.voice_webhook_url, payload, timeout_sec=self.server.timeout_sec)
                    memory_persisted = persist_memory_summary_if_allowed(
                        event=event,
                        user_id=user_id,
                        memory_state=memory_state,
                        memory_state_path=self.server.memory_state_file,
                        memory_policy=self.server.memory_policy,
                        summary_text=extract_memory_summary_from_result(result),
                    )
                    decision = "forwarded"
                    reason = "voice_forwarded"
                else:
                    result = build_voice_scaffold_payload(event, command, self.server.voice_webhook_url, self.server.voice_forward)
                    decision = "ok"
                    reason = "voice_scaffold"
                voice_cooldown_mark(voice_state, event, command)
                save_voice_state(self.server.voice_state_file, voice_state)
                append_audit(
                    self.server.audit_log,
                    {
                        "ts": utc_now_iso(),
                        ...
                        "decision": decision,
                        "reason": reason,
                        "webhook_url": self.server.voice_webhook_url,
                        "guild_id": normalize_id(event.get("guild_id")),
                        "channel_id": normalize_id(event.get("channel_id") or event.get("chat_id")),
                        "user_id": user_id,
                        "role": str(event.get("role") or "user"),
            ...
            if command == "memory":
                ...
                self._json(200 if decision == "ok" else 409, result)
                return
            ...
            if command == "ops" and str(payload.get("role") or "user").lower() != "admin":
                denial = {
                    "reply": "Ops command denied: admin role required.",
                    "route": "discord-ops-admin-only",
                }
                append_audit(
                    self.server.audit_log,
                    {
            ...
            webhook_url = self.server.rag_webhook_url if command == "ask" else self.server.ops_webhook_url
            result = post_json(webhook_url, payload, timeout_sec=self.server.timeout_sec)
            memory_persisted = False
            if command == "ask":
                memory_persisted = persist_memory_summary_if_allowed(
                    event=event,
            ...
            append_audit(
                ...
                {
                    "ts": utc_now_iso(),
                    "command": str(event.get("command") or "ask"),
                    "decision": "error",
                    "reason": "http_error",
            ...
            self._json(int(getattr(exc, "code", 502)), {"error": "proxy_failed", "detail": str(exc)})
            return
        except Exception as exc:
            append_audit(
                self.server.audit_log,
            ...
            self._json(500, {"error": "proxy_failed", "detail": str(exc)})
            return
...
class ProxyServer(ThreadingHTTPServer):
    def __init__(
        ...
        *,
        ...
        self.rag_webhook_url = rag_webhook_url
        self.ops_webhook_url = ops_webhook_url
        self.n8n_base = n8n_base
        self.n8n_health_path = n8n_health_path
        self.voice_webhook_url = voice_webhook_url
        self.voice_forward = voice_forward
        self.voice_state_file = voice_state_file
        self.memory_state_file = memory_state_file
        self.memory_policy = memory_policy
        self.voice_cooldown_seconds = voice_cooldown_seconds
        self.voice_moderator_role_ids = voice_moderator_role_ids
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Local HTTP wrapper for Discord text command proxy")
    ...
    rag_webhook_url = args.n8n_base.rstrip("/") + rag_webhook
    ops_webhook_url = args.n8n_base.rstrip("/") + args.ops_webhook
    ...
    httpd = ProxyServer(
        ...
        n8n_health_path=args.n8n_health_path,
        voice_webhook_url=voice_webhook_url,
        voice_forward=bool(args.voice_forward),
        voice_state_file=Path(args.voice_state_file),
        memory_state_file=Path(args.memory_state_file),
        memory_policy=memory_policy,
        voice_cooldown_seconds=voice_cooldown_seconds,
        voice_moderator_role_ids=parse_id_csv(args.voice_moderator_role_ids),
    ...
    print(
        "discord-rag-proxy-server listening on "
        f"http://{args.host}:{args.port} -> ask:{rag_webhook_url} ops:{ops_webhook_url} voice:{voice_webhook_url}"
    )
    httpd.serve_forever()
